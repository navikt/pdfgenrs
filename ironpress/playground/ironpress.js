...
export function htmlToPdf(html) {
    ...
    const ret = wasm.htmlToPdf(ptr0, len0);
    if (ret[2]) {
 ...
 * @returns {Uint8Array}
 */
export function htmlToPdfCustom(html, page_width, page_height, margin_top, margin_right, margin_bottom, margin_left) {
    const ptr0 = passStringToWasm0(html, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.htmlToPdfCustom(ptr0, len0, page_width, page_height, margin_top, margin_right, margin_bottom, margin_left);
    if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
    }
    return takeFromExternrefTable0(ret[0]);
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('ironpress_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
